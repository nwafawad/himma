...
import type { ImportCandidate, ImportStats } from "@himma/contracts";
import {
  confirmImport,
...
export default function ImportModal() {
  ...
  const handleIngestSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const message = getErrorMessage(err, "Failed to process import file. Please verify file format.");
      if (message.includes("INVALID_") || message.includes("NO_VALID_ENTRIES") || message.includes("400") || message.includes("422")) {
  ...
  const updateCandidate = (id: string, field: "title" | "type", value: string) => {
    setCandidates((prev) =>
      prev.map((candidate) => {
        ...
        return { ...candidate, type: value as ImportCandidate["type"] };
      })
    );
  };
