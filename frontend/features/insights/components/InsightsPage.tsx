...
import FeedbackModal from "@/features/insights/components/FeedbackModal";
import { generateInsight, listInsights, submitInsightFeedback } from "@/features/insights/api";
import { getErrorMessage } from "@/lib/errors";

export default function InsightsPage() {
  const [insightRuns, setInsightRuns] = useState<Insight[]>([]);
  ...
  const loadInsights = useCallback(async () => {
    ...
    try {
      ...
      setInsightRuns(response.data);
    } catch (err: unknown) {
      console.warn("Could not fetch insights from backend API:", err);
      setErrorMessage(getErrorMessage(err, "Unable to load career insights."));
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const handleGenerateInsight = async () => {
    ...
    try {
      ...
      setNotification({
        type: "error",
        message: getErrorMessage(err, "Failed to trigger AI insight generation."),
      });
    } finally {
      setGenerating(false);
    }
  ...
  const handleConfirmFeedback = async (insightId: string) => {
    try {
      await submitInsightFeedback(insightId, { action: "confirm" });
      ...
      setNotification({
        type: "error",
        message: getErrorMessage(err, "Failed to submit feedback."),
      });
    }
  };
  ...
  const getStatusBadge = (score: Insight["alignmentScore"], status: string) => {
    if (status === "skipped") {
      return (
