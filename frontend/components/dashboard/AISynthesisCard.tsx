...
interface InsightData {
  id?: string;
  timestamp?: string;
  alignmentScore?: "on_track" | "drifting" | "no_stated_goal" | "on track" | "no stated goal yet";
  status?: string;
  statusReason?: string;
  inputWindow?: {
...
export default function AISynthesisCard() {
  ...
  const loadLatestInsight = useCallback(async () => {
    ...
    try {
      ...
      const [insightsRes, activitiesRes, notesRes] = await Promise.all([
        fetchApi<{ data?: InsightData[] }>("/insights?limit=1").catch(() => null),
        fetchApi<{ data?: any[] }>("/activities?limit=20").catch(() => null),
        fetchApi<{ data?: any[] }>("/notes?limit=20").catch(() => null),
      ]);
      ...
      if (latestRun && latestRun.citations && latestRun.citations.length > 0) {
        ...
        const actMap = new Map(rawActivities.map((a: any) => [a.id, a]));
        const noteMap = new Map(rawNotes.map((n: any) => [n.id, n]));
  ...
  const handleGenerateSynthesis = async () => {
    ...
    setErrorMsg("");
    try {
      const res = await fetchApi<{ skipped?: boolean; reason?: string; data?: InsightData }>("/insights/generate", {
        method: "POST",
      ...
      setErrorMsg(err.message || "Failed to generate AI synthesis. Please try again.");
    } finally {
      setGenerating(false);
    }
  };
